...
local screen_splash_gradient = starcolor.gradient(gradient_start, gradient_end, banner_line_count)
local screen_splash_color_group = starcolor.text_color_group_from_seqs('screen_splash', { fg = screen_splash_gradient })

vim.cmd(':54vsplit '..vim.fn.stdpath('config')..'/'..screen_content_file)
local screen_buffer = vim.fn.bufnr()

starcolor.color_lines_seq(screen_splash_color_group, screen_buffer, banner_start_line, banner_line_count)
